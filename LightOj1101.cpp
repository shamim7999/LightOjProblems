...
int parent[mx];
int n,danger=-1;
int par[mx][24],level[mx], dis[mx][24];
...
void dfs(int node, int baba)
{
    ...
    for(auto it : g[node]) if(it.fr!=baba) dis[it.fr][0] = it.se, dfs(it.fr,node);
}
...
void solve(int tc)
{
    ...
    int edge;
    scanf("%d %d", &n, &edge);
    for(int i=0,n1,n2,w; i<edge; i++) scanf("%d %d %d", &n1, &n2, &w), graph.pb({w,{n1,n2}});
    ini();
    kruskal();
    memo(par,-1);
    memo(dis,0);
    ...
    for(int j=1; j<=22; j++){
        for(int i=1; i<=n; i++){
            if(par[i][j-1]!=-1) par[i][j] = par[par[i][j-1]][j-1],dis[i][j]=max(dis[i][j-1],dis[par[i][j-1]][j-1]);
            else par[i][j]=-1;
        }
    }
    int q;
